...
/**
 * @author Seeker
 * @date 2019/1/17/017  16:12
 * @describe 展示loading的，支持颜色、大小、转速的设置
 */
public class LoadingView extends View {
    ...
    public LoadingView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        mSize = array.getDimensionPixelSize(R.styleable.LoadingView_viewSize, DisplayHelper.dp2px(context, 32));
        mPaintColor = array.getColor(R.styleable.LoadingView_android_color,0xFF5A5A5A);
        mDuration = array.getInt(R.styleable.LoadingView_android_duration,600);
        array.recycle();
        initPaint();
    }

    public LoadingView(Context context, int size, int color,long duration) {
        super(context);
        mSize = size;
        mPaintColor = color;
        mDuration = duration;
